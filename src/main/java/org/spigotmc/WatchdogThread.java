...
public class WatchdogThread extends Thread {
   ...
   public void run() {
      while(!this.stopping) {
         if (this.lastTick != 0L && this.timeoutTime > 0L && monotonicMillis() > this.lastTick + this.timeoutTime) {
            ...
            dumpThread(ManagementFactory.getThreadMXBean().getThreadInfo(MinecraftServer.getServer().ag.getId(), Integer.MAX_VALUE), log);
            log.log(Level.SEVERE, "------------------------------");
   ...
   private static void dumpThread(ThreadInfo thread, Logger log) {
      ...
      if (thread.getLockedMonitors().length != 0) {
         ...
         MonitorInfo[] var5;
         for(MonitorInfo monitor : var5 = thread.getLockedMonitors()) {
      ...
      for(StackTraceElement stack : var9 = thread.getStackTrace()) {
         log.log(Level.SEVERE, "\t\t" + stack);
      }
   }
}
