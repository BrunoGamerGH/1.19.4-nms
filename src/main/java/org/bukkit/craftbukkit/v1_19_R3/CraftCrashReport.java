   ...
   public String get() {
      ...
      try {
         value.append("\n   Running: ")
            ...
            .append(" (Implementing API version ")
            .append(Bukkit.getBukkitVersion())
            .append(") ")
            .append(String.valueOf(MinecraftServer.getServer().U()));
         value.append("\n   Plugins: {");

         Plugin[] var5;
         for(Plugin plugin : var5 = Bukkit.getPluginManager().getPlugins()) {
         ...
         for(Entry<Thread, ? extends Object[]> entry : Thread.getAllStackTraces().entrySet()) {
            value.append(' ')
               .append(entry.getKey().getState().name())
               .append(' ')
         ...
         for(World world : Bukkit.getWorlds()) {
            ...
            for(Entry<Plugin, Collection<Chunk>> entry : world.getPluginChunkTickets().entrySet()) {
               value.append(' ')
                  .append(((Plugin)entry.getKey()).getDescription().getFullName())
                  .append(": ")
                  .append(Integer.toString(entry.getValue().size()))
                  .append(',');
            }
