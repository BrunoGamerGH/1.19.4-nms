...
public class DataConverterFlatten extends DataFix {
   private static final Map<String, String> a = (Map<String, String>)DataFixUtils.make(Maps.newHashMap(), hashmap -> {
   ...
   private static final Set<String> b = a.keySet().stream().map(s -> s.substring(0, s.indexOf(46))).collect(Collectors.toSet());
   ...
   public TypeRewriteRule makeRule() {
      ...
      OpticFinder<Pair<String, String>> opticfinder = DSL.fieldFinder("id", DSL.named(DataConverterTypes.s.typeName(), DataConverterSchemaNamed.a()));
      ...
      return this.fixTypeEverywhereTyped("ItemInstanceTheFlatteningFix", type, typed -> {
         ...
         if (!optional.isPresent()) {
            ...
            if (s != null) {
               typed1 = typed.set(opticfinder, Pair.of(DataConverterTypes.s.typeName(), s));
            }
